...
we can use two frequencies, each targeting to optimize the trapping of one species. Higher frequency $\Omega_2$ with amplitude $V_2$ for electron trapping. A lower frequency $\Omega_1$ with potential on electrodes $V_1$ for trapping ions. We have already derived the ideal quadrupole potential for a single frequency case. Repeating the same process with the exception of using applied potential on the electrodes in the form $\Phi_0 = V_0 + V_1 \cos(\Omega_1 t) + V_2 \cos(\Omega_2 t)$, yields two-frequency potential for an ideal quadrupole:
\begin{equation}
...
\end{equation}
The equations of motion in such a potential, after a change of variable: $\tau = \nicefrac{t\Omega_2}{2}$ are:
\begin{subequations}
...
\end{equation}
where $\tensorq{T}$ denotes a matrix valued function with minimal period $T$. Let's illustrate this theory for the case of our differential equation. We begin by rewriting the equation \eqref{eq of motion for simulation} as one general system of two first-order differential equations written in the matrix form:
\begin{align}
...
\end{equation}
where $\chi$ is a dispersion angle. The maximal change in kinetic energy happens for $\chi = \pi$. Considering helium atoms at a room temperature $T = \SI{300}{\kelvin}$, the value of this energy change is $\Delta E = \SI{0.16}{\kelvin}$, which would be enough to kick a weakly confined electron out of the trap\footnote{In our experiment, we will be able to create an effective potential well for ions with a maximum depth of $\SI{32}{\milli\kelvin}$, and for electrons with maximal depth up to $\SI{15}{\kelvin}$.}.
\end{description}	

\subsection{Time scales}
When looking at the electrons' stability, we use a total simulation length corresponding to twenty secular oscillations $(\nicefrac{20}{\omega})$ \eqref{secular frequency}, if we do not say otherwise. When examining the average velocity of an electron, it was sometimes necessary to extend this time up to two-hundred secular oscillations to harvest the practically relevant information. Choice of time-step is always a delicate issue. We used a constant time-step for the case of a single electron in the exact potential, following the Nyquist criterion \cite{1697831}. Nyquist criterion is used primarily in processing compact signals with convergent Fourier series. It states that for such signal with the highest frequency $\textit{f} \equiv \nicefrac{\Omega}{2\pi}$, the largest possible sample size so that the discretization of the signal will carry equivalent information as the continuous one is $\nicefrac{1}{(2\textit{f})}$. We started numerically solving the exact equation of motion with this time step, gradually decreasing its size until the numerical solution with the denser sampling would give us the same result for a given tolerance. Repeating this process with several numerical methods \cite{teukolsky1992numerical} for integrating ODEs, we have picked the most efficient one. For the case of a single electron in the trap, the best performance had a predictor-corrector method. The sufficient time-step was $\Delta t = \nicefrac{1}{(10 \Omega)}$. For simulations including multiple particles, we have used the Verlet method.
