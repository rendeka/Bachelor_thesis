...
def ODESystemEffective(rv, t, aCoulomb, mass, charge, trapParams):
    ...
    x1 = vx
    vx1 = aCoulomb[0] - mass * x / 4 * (a + q1**2 / 2 * (f2 / f1)**2 + q2**2 / 2) * f2**2
    
    y1 = vy
    vy1 = aCoulomb[1] - mass * y / 4 * (a + q1**2 / 2 * (f2 / f1)**2 + q2**2 / 2) * f2**2
    
    z1 = vz
    vz1 = aCoulomb[2] - mass * z * (a + q1**2 / 2 * (f2 / f1)**2 + q2**2 / 2) * f2**2
...
def ODESystemEffectiveDamping(rv, tau, aCoulomb, mass, charge, trapParams): #effective potential
    ...
    x1 = vx
    vx1 = aCoulomb[0] - mass * x / 4 * (a + q1**2 / 2 * (f2 / f1)**2 + q2**2 / 2) * f2**2 - beta * vx
    
    y1 = vy
    vy1 = aCoulomb[1] - mass * y / 4 * (a + q1**2 / 2 * (f2 / f1)**2 + q2**2 / 2) * f2**2 - beta * vy
    
    z1 = vz
    vz1 = aCoulomb[2] - mass * z * (a + q1**2 / 2 * (f2 / f1)**2 + q2**2 / 2) * f2**2 - beta * vz
...
def ODESystemDampingExact(rv, tau, aCoulomb, mass, charge, trapParams):
    ...
    x1 = vx
    vx1 = aCoulomb[0] - x * (a - 2 * q1 * np.cos(2 * tau * f1 / f2) - 2 * q2 * np.cos(2 * tau)) - beta * vx
    
    y1 = vy
    vy1 = aCoulomb[1] - y * (a - 2 * q1 * np.cos(2 * tau * f1 / f2) - 2 * q2 * np.cos(2 * tau)) - beta * vy
    
    z1 = vz
    vz1 = aCoulomb[2] + 2 * z * (a - 2 * q1 * np.cos(2 * tau * f1 / f2) - 2 * q2 * np.cos(2 * tau)) - beta * vz
